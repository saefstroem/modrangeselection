...
- However, since a range must contain at least one unselected ID, and each split consumes one selected ID, the **total number of IDs** satisfies:
    ...
    Each range has at least one unselected ID, so the number of unselected IDs is at least \( R \). The number of selected IDs is \( s \). Therefore:
    $n \geq R + s$

    But from the earlier equation, $R = 1 + s$, so:
...
-  **Conclusion**: The maximum number of ranges $R$ is at most $\left\lceil  \dfrac{n}{2} \right\rceil$.
